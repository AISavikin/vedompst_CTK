...
class MarkKidsWindow(Mixin, ctk.CTkToplevel):
    ...
    def make_screenshot(self):
        ...
        path += f'/{self.date:%d}-Группа_{self.group}.jpg'
...
class KidsFrame(ctk.CTkFrame):
    ...
    def navigation(self, event):
        ...
        if event.keysym == 'Left':
            custom_focus.focus_element.delete(0, END)
            custom_focus.focus_element.insert(0, 'н')
        if event.keysym == 'Right':
            custom_focus.focus_element.delete(0, END)
            custom_focus.focus_element.insert(0, 'б')
        custom_focus.set()
